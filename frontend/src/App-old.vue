...
const selectedShape = ref<any | null>(null);

const handleToolSelected = (tool: ToolType) => {
  selectedTool.value = tool;
};

// Watch for shape selection to auto-switch to select tool and sync color
watch(selectedShape, (newShape) => {
  // Auto-switch to select tool when a shape is selected
  if (newShape && selectedTool.value !== 'select') {
    selectedTool.value = 'select';
  }
  // Sync color picker to selected shape's color
  if (newShape && newShape.color) {
    currentColor.value = newShape.color;
  }
});

// Watch for color changes and update selected shape
watch(currentColor, (newColor) => {
  if (selectedShape.value) {
    handleUpdateShapeProperty(selectedShape.value.id, 'color', newColor);
  }
});

const handleUpdateShapeProperty = (
  shapeId: string,
  property: string,
  value: any
) => {
  if (canvasRef.value) {
    canvasRef.value.updateShapeProperty(shapeId, property as any, value);
  }
  // Update selected shape reference to reflect changes
  if (selectedShape.value && selectedShape.value.id === shapeId) {
    (selectedShape.value as any)[property] = value;
  }
};
...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  if (event.key === 'Backspace' || event.key === 'Delete') {
    if (canvasRef.value && selectedShape.value) {
...
<template>
  <div class="h-screen flex flex-col font-sans bg-gray-50 dark:bg-gray-900">
    ...
    <PropertyPanel
      ...
      @update-shape-property="handleUpdateShapeProperty"
    />
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <Canvas
        ...
        :font-size="fontSize"
        :font-family="fontFamily"
        v-model:selected-shape="selectedShape"
      />
    </div>
  </div>
</template>
