...
export function generateShapeId(): string {
  return `shape-${Date.now().toString(36)}`;
}
...
function perpendicularDistance(
  ...
  const delta = lineEnd.minus(lineStart);
...
export function findClosestSegment(
  points: Point[],
  clickPoint: Point
): { insertIndex: number; projectionPoint: Point } {
  let minDistance = Infinity;
  let insertIndex = 1; // Default to after first point
  let projectionPoint = new Point(0, 0);
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    if (distance < minDistance) {
      ...
      insertIndex = i + 1;
      projectionPoint = projectPointOntoLineSegment(
        clickPoint,
        points[i],
        points[i + 1]
      );
